...
constructor(private val context: Context) {
    ...
    fun notMatchString(s: String): MessageBuilder {
        return MessageBuilder(Type.NOT_MATCH, s)
    }
        ...
        for (builder in messageBuilders) {
            ...
            try {
                when (builder.type) {
                    ...
                    Type.NOT_MATCH -> notMatch(subject, builder.match, builder.message)
    ...
    @Throws(ApplicationException::class)
    internal fun notMatch(subject: String, toMatch: String?, errorMessage: String?) {
        if (toMatch == null || subject == toMatch)
            throw ApplicationException(errorMessage!!)
    }
    ...
    enum class Type {
        EMPTY, EMAIL, MIN_LENGTH, MAX_LENGTH, MATCH,NOT_MATCH, PATTERN_MATCH, EMPTY_WITHOUT_TRIM
    }
