...
@HiltViewModel
class PatientDetailViewModel @Inject constructor(private val userRepository: UserRepository) :
    BaseViewModel() {

    val patientDetailLiveData = APILiveData<User>()

    fun patientDetails() {
        viewModelScope.launch {
            val result = userRepository.getPatientDetails()
            patientDetailLiveData.value = result
        }
    }
}