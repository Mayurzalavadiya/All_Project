...
//        locationAdapter.addItem(getLatLng())
...
//                    val iconBitmap = mIconGenerator.makeIcon()
    ...
    private fun moveCamera(latLngData: LatLng, zoom: Float, bearing: Float = 0f) {
        val cameraPosition = CameraPosition.Builder().apply {
            target(latLngData) // Marker position // Rotate the map
            bearing(bearing)
    ...
    private fun moveMarkerRecursively(index: Int) {
        ...
        handler.post(object : Runnable {
            override fun run() {
                ...
                val rotation = getBearing(startPosition, finalPosition)
                carMarker?.rotation = rotation // Update car rotation


//                if (!isZoom) {
                moveCamera(currentPosition, 9f, rotation)
//                }
                if (t < 1) {
    ...
    private fun getBearing( startPosition: LatLng, finalPosition: LatLng): Float {
