...
import com.starter.app.databinding.MyPostListBinding
...
class MyPostAdapter(val editListener: EditListener) :
    ...
    fun addItems(itemList: MutableList<MyQuotes>) {
        this.quotesList.addAll(itemList)
        notifyDataSetChanged()
    }

    @SuppressLint("NotifyDataSetChanged")
    fun addItem(itemList: MyQuotes?) {
        if (itemList != null) {
            this.quotesList.add(itemList)
        }
        notifyItemInserted(quotesList.size)
    }
