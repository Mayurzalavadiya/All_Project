...
import com.paymentgetway.app.core.AESCryptoInterceptor
...
import com.paymentgetway.app.exception.AuthenticationException
import com.paymentgetway.app.exception.ServerError
...
//            .addInterceptor(httpLoggingInterceptor).addInterceptor(aesInterceptor)
    ...
    @Singleton
    @Provides
    @Named(DiConstants.EPHEMERAL_KEYS)
    internal fun provideClient1(
//        @Named(DiConstants.EPHEMERAL_KEYS) headerInterceptor: Interceptor,
        @Named(DiConstants.PRE_VALIDATION) networkInterceptor: Interceptor,
    ): OkHttpClient {
        val httpLoggingInterceptor = HttpLoggingInterceptor()
        httpLoggingInterceptor.level = HttpLoggingInterceptor.Level.BODY
    ...
    @Singleton
    @Named(DiConstants.EPHEMERAL_KEYS)
    internal fun provideRetrofit1(@Named(DiConstants.EPHEMERAL_KEYS) okHttpClient: OkHttpClient): Retrofit {
        return Retrofit.Builder().baseUrl(URLFactory.provideHttpUrl()).client(okHttpClient)
            .addConverterFactory(GsonConverterFactory.create()).build()
    }
    ...
    @Singleton
    @Named(DiConstants.EPHEMERAL_KEYS)
    internal fun provideHeader(session: Session): Interceptor {
        return Interceptor { chain ->
            val build = chain.request().newBuilder().addHeader(Session.API_KEY, session.apiKey)
                .build()
            chain.proceed(build)
