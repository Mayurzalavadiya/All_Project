...
@HiltViewModel
class GetPaymentViewModel @Inject constructor(private val userRepository: UserRepository) : BaseViewModel() {

    val paymentLiveData = MutableLiveData<GetPayment>()

    fun payment(customer: String) {
        viewModelScope.launch {
            val result = userRepository.getPaymentIntents(customer)
            paymentLiveData.value = result
        }
    }
}